...
public class FrameVisualsProvider
  ...
  public Image getIcon( IFrame aItem, EIconSize aIconSize ) {
    ...
    TsImage tsImg = psxFileSystem().findThumb( aItem, thumbSize );
    if( tsImg != null ) {
      return tsImg.image();
    }
    return null;
  ...
  public TsImage getThumb( IFrame aItem, EThumbSize aThumbSize ) {
    return aItem != null ? psxFileSystem().findThumb( aItem, aThumbSize ) : null;
  }
  ...
  public TsImage getFullSizeImage( IFrame aItem ) {
    File f = psxFileSystem().findFrameFile( aItem );
    if( f.exists() ) {
