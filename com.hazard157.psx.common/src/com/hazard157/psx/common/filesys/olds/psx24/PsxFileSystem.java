...
public class PsxFileSystem
  ...
  static final String ORIGINAL_MEDIA_ROOT = ROOT_DIR + "mastering/original/"; //$NON-NLS-1$
  ...
  public PsxFileSystem( IEclipseContext aWinContext ) {
    ...
    framesCache = new FramesCache( this );
  ...
  public boolean createGifAnimation( String aEpisodeId, String aCamId, int aStartSec ) {
    ...
    IRunnableWithProgress op = aMonitor -> {
      ...
      try {
        ...
        // обновим кеш
        framesCache.frameFileAdded( new Frame( aEpisodeId, aCamId, aStartSec * FPS, true ) );
        status = true;
      }
      catch( Exception e ) {
        e.printStackTrace();
