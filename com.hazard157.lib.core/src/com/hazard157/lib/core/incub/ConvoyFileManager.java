...
public class ConvoyFileManager {
  ...
  public boolean isConvoyFile( String aFileName ) {
    ...
    String ext = TsFileUtils.extractExtension( aFileName );
    String bare = TsFileUtils.extractBareFileName( aFileName );
  ...
  public final String formatConvoyFileName( String aFileName ) {
    TsErrorUtils.checkNonEmpty( aFileName );
    TsIllegalArgumentRtException.checkTrue( isConvoyFile( aFileName ) );
    return CHAR_DOT + TsFileUtils.extractFileName( aFileName ) + CHAR_DOT + convoyFileExt;
  }
