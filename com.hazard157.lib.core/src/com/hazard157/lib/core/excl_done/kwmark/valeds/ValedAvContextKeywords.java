package com.hazard157.lib.core.excl_done.kwmark.valeds;

import static com.hazard157.lib.core.excl_done.kwmark.valeds.IHzResources.*;
 ...
 * {@link IKeywordManager#listKeywords()} в выпадающем списке или ввести новый и нажать "Добавить".
 *
 * @author hazard157
 */
public class ValedAvContextKeywords
    extends AbstractValedControl<IAtomicValue, Control> {

  /**
   * The factory name.
   */
  public static final String FACTORY_NAME = VALED_EDNAME_PREFIX + ".ValedAvContextKeywords"; //$NON-NLS-1$
  ...
  static class Factory
    ...
    protected IValedControl<IAtomicValue> doCreateEditor( ITsGuiContext aContext ) {
      AbstractValedControl<IAtomicValue, Control> e = new ValedAvContextKeywords( aContext );
      e.setParamIfNull( IValedControlConstants.OPID_IS_HEIGHT_FIXED, AV_FALSE );
      e.setParamIfNull( IValedControlConstants.OPID_IS_WIDTH_FIXED, AV_FALSE );
      e.setParamIfNull( IValedControlConstants.OPID_VERTICAL_SPAN, avInt( 5 ) );
      return e;
    }
   ...
   */
  public ValedAvContextKeywords( ITsGuiContext aTsContext ) {
    super( aTsContext );
  }
  ...
  private void fillComboItems() {
    IKeywordManager kwMan = tsContext().get( IKeywordManager.class );
    comboWidget.removeAll();
    for( String kw : kwMan.listKeywords() ) {
      comboWidget.add( kw );
    }
  }
  ...
  void whenBtnAddPressed() {
    ...
    // FIXME add new keyword to the manager
