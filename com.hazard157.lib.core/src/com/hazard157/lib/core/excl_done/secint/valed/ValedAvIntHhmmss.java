...
public class ValedAvIntHhmmss
  ...
  IMpvSecsDuration     hmsMpv    = null;
  MultiPartValueWidget hmsWidget = null;
  ...
  private void recreateWidgets() {
    ...
    try {
      ...
      if( hmsWidget != null ) {
        value = avInt( hmsMpv.getValueSecs() );
        hmsWidget.dispose();
        hmsWidget = null;
        hmsMpv = null;
      }
      if( isEditable() ) {
        hmsMpv = IMpvSecsDuration.create( !getIsOnlyMmss(), true );
        hmsWidget = new MultiPartValueWidget( backplane, SWT.NONE, hmsMpv );
        updateWidgetLimits();
  ...
  private void updateWidgetLimits() {
    ...
    hmsMpv.setRange( new IntRange( minValue, maxValue ) );
  }
  ...
  private void displayValue() {
    ...
    if( hmsWidget != null ) {
      if( value.isAssigned() ) {
        hmsMpv.setValueSecs( value.asInt() );
      }
      else {
        hmsMpv.setValueSecs( getMinValue() );
      }
    }
  }
